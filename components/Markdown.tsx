...
import { replaceEmojis } from "../tools/emoji_util";
    ...
    marked.use({
      renderer: ({
        ...
        image(href, title, text) {
          ...
          return `<img ${url ? `src="${imgs_secman}${url}"` : ""} ${
            text ? `alt="${text}"` : ""
        ...
        html(html: string) {
          ...
          images.forEach((a) => {
            ...
            html = html.replace(`src="${imgs_secman}${original}"`, `src="${imgs_secman}${final}"`);
          });
...
function transformLinkUri(displayURL: string, baseURL: string) {
  return (uri: string) => {
    ...
    if (
      ...
      hrefURL?.origin === "https://secman.web.app"
    ) {
